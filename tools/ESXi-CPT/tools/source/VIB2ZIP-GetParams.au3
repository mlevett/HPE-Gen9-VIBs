...
#AutoIt3Wrapper_Res_Field=ProductVersion|2.4
...
$GUI = GUICreate("VIB2ZIP - ESXi-CPT.v-front.de - Modded to support ESXi 8 by Mathew Levett", 500, 480, 250, 250, $WS_CAPTION + $WS_SYSMENU + $WS_MINIMIZEBOX + $WS_SIZEBOX)
...
$mradio[4] = GUICtrlCreateCheckbox("8.*", 40, 0)
$mradio[5] = GUICtrlCreateCheckbox("8.0", 40, 0) ; was [4]
$mradio[6] = GUICtrlCreateCheckbox("8.5", 40, 0) ; was [5]
$mradio[7] = GUICtrlCreateCheckbox("8.7", 40, 0) ; was [6]
...
Func LoadSettingsFromIni($LIniFile)
   ...
   Global $obESXiVer6x = IniRead($LIniFile, "Settings", "obESXiVer6x", "1")
   Global $obESXiVer60 = IniRead($LIniFile, "Settings", "obESXiVer60", "1")
   Global $obESXiVer65 = IniRead($LIniFile, "Settings", "obESXiVer65", "1")
   Global $obESXiVer67 = IniRead($LIniFile, "Settings", "obESXiVer67", "1")
   SetCheckBox($mRadio[0], $obESXiVer5x, 0)
   SetCheckBox($mRadio[4], $obESXiVer6x, 0)
   ...
   EndIf
   if $obESXiVer6x = "1" Then
      $obESXiVer60 = 1
      $obESXiVer65 = 1
      $obESXiVer67 = 1
	  SetCheckBox($mRadio[5], $obESXiVer60, $GUI_DISABLE)
	  SetCheckBox($mRadio[6], $obESXiVer65, $GUI_DISABLE)
	  SetCheckBox($mRadio[7], $obESXiVer67, $GUI_DISABLE)
   Else
	  SetCheckBox($mRadio[5], $obESXiVer60, 0)
	  SetCheckBox($mRadio[6], $obESXiVer65, 0)
	  SetCheckBox($mRadio[7], $obESXiVer67, 0)
   EndIf
EndFunc
...
While 1
	...
	If GUICtrlRead($VD_Input) <> "" AND GUICtrlRead($WD_Input) <> "" AND GUICtrlRead($obName) <> "" AND GUICtrlRead($obVersion) <> "" _
	   ...
	   AND ((GUICtrlRead($mRadio[0]) = $GUI_CHECKED) OR (GUICtrlRead($mRadio[1]) = $GUI_CHECKED) OR (GUICtrlRead($mRadio[2]) = $GUI_CHECKED) OR (GUICtrlRead($mRadio[3]) = $GUI_CHECKED) OR (GUICtrlRead($mRadio[4]) = $GUI_CHECKED) OR (GUICtrlRead($mRadio[5]) = $GUI_CHECKED) OR (GUICtrlRead($mRadio[6]) = $GUI_CHECKED) OR (GUICtrlRead($mRadio[7]) = $GUI_CHECKED)) Then
	...
	if $focus <> $newfocus Then
		...
		$focus = $newfocus
    EndIf
	; Handle ESXi 5 version selection changes
	...
	; Handle ESXi 6 version selection changes
	if BitAND(GUICtrlRead($mradio[4]), $GUI_CHECKED) = 0 Then
		$obESXiVer6xNEW = 0
	Else
		$obESXiVer6xNEW = 1
	EndIf
	; If Wildcard (6.*) was toggled:
	If $obESXiVer6xNEW <> $obESXiVer6x Then
		$obESXiVer6x = $obESXiVer6xNEW
		If $obESXiVer6x = 0 Then
			$obESXiVer60 = 1
			$obESXiVer65 = 0
			$obESXiVer67 = 0
			SetCheckBox($mRadio[5], $obESXiVer60, $GUI_ENABLE)
			SetCheckBox($mRadio[6], $obESXiVer65, $GUI_ENABLE)
			SetCheckBox($mRadio[7], $obESXiVer67, $GUI_ENABLE)
		Else
			$obESXiVer60 = 1
			$obESXiVer65 = 1
			$obESXiVer67 = 1
			...
			SetCheckBox($mRadio[7], $obESXiVer67, $GUI_DISABLE)
		EndIf
	; If all of 6.0, 6.5 and 6.7 selected then activate wildcard 6.* (if not already active):
	ElseIf (($obESXiVer6x = 0) AND (BitAND(GUICtrlRead($mradio[5]), $GUI_CHECKED) = $GUI_CHECKED) _
							   AND (BitAND(GUICtrlRead($mradio[6]), $GUI_CHECKED) = $GUI_CHECKED) _
							   AND (BitAND(GUICtrlRead($mradio[7]), $GUI_CHECKED) = $GUI_CHECKED)) Then
		$obESXiVer6x = 1
		$obESXiVer60 = 1
		$obESXiVer65 = 1
		$obESXiVer67 = 1
	...
	Select
		...
		case $msg = $Launcher
			...
			IniSaveCheckbox("Settings", "obESXiVer6x", GUICtrlRead($mRadio[4]))
			IniSaveCheckbox("Settings", "obESXiVer60", GUICtrlRead($mRadio[5]))
			IniSaveCheckbox("Settings", "obESXiVer65", GUICtrlRead($mRadio[6]))
			IniSaveCheckbox("Settings", "obESXiVer67", GUICtrlRead($mRadio[7]))
			...
			Else
				...
				CmdSaveCheckbox("obESXiVer6x", GUICtrlRead($mRadio[4]))
				CmdSaveCheckbox("obESXiVer60", GUICtrlRead($mRadio[5]))
				CmdSaveCheckbox("obESXiVer65", GUICtrlRead($mRadio[6]))
				CmdSaveCheckbox("obESXiVer67", GUICtrlRead($mRadio[7]))
				FileClose($parFile)
				$ExitCode = 0
			EndIf
			ExitLoop
